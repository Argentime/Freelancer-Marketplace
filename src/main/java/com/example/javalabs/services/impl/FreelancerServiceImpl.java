...
public class FreelancerServiceImpl implements FreelancerService {
    ...
    // Вспомогательный метод для поиска навыка по имени (добавим в SkillRepository)
    private Optional<Skill> findSkillByName(String name) {
        return skillRepository.findByName(name); // Предполагается, что в SkillRepository есть метод findByName
    }
}